...
instance toHtmlBlock :: ToHtml Block where
  ...
  toHtml (CodeBlock (Fenced _ info) ss) = pre ! className "code line-numbers" $ for_ ss (\s -> code ! className ("code-line language-" <> info) $ text s)
  toHtml (CodeBlock _ ss)            = pre ! className "code line-numbers " $ for_ ss (\s -> code ! className "code-line language-haskell" $ text s)
...
instance toHtmlInline :: ToHtml Inline where
  ...
  toHtml Space                       = text " "
  toHtml SoftBreak                   = htmlEntity "&shy;"
  toHtml LineBreak                   = br
